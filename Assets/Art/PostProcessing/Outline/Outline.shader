Shader "Hidden/Outline Post Process"
{
	SubShader
	{
		...
		// _CameraNormalsTexture contains the view space normals transformed to be in the 0...1 range.
		...
		// Both the Varyings struct and the Vert shader are copied from StdLib.hlsl included above, with some modifications.
		struct Varyings
		{
		...
		Varyings Vert(AttributesDefault v)
		{
			...
			// Transform our point first from clip to view space, taking the xyz to interpret it as a direction.
		...
		float4 Frag(Varyings i) : SV_Target
		{
			...
			// The above variables ensure offset exactly one pixel at a time.
			...
			// Return a value in the 0...1 range depending on where NdotV lies between _DepthNormalThreshold and 1.
			...
			// Dot the finite differences with themselves to transform to scalars.
			...
			float4 rightNeighbor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, (i.texcoord + float2(_MainTex_TexelSize.x, 0)));
			float4 leftNeighbor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, (i.texcoord - float2(_MainTex_TexelSize.x, 0)));
			float4 topNeighbor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, (i.texcoord + float2(0, _MainTex_TexelSize.y)));
			...
			{
				edgeColor = float4(_Light.rgb, _Light.a * edge);
			}
